from pavimentados2.models.yolo import YoloV3
...
class Yolo_Model(Pav_Model):
	...
	def load_model(self):
		...
		with tf.device(self.device):
			...
			# Load the pavement model classes.
			...
			# Link the classes of both models.
			self.full_classes = [*classes_paviment, *classes_signal]

			# Load the path of the weights of pavement and sign weights.
...
class Siamese_Model(Pav_Model):
	...
	def load_model(self):
		...
		with tf.device(self.device):
			...
			print('Creating the Siamese model')
