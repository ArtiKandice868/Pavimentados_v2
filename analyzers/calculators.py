from pavimentados2.analyzers.utils import total_distance, area_calc, area_calc_meters, box_center, box_height, box_width, fail_id_generator, stack_columns_dataset,  assign_group_calculations
import numpy as np
import pandas as pd
...
class Results_Calculator:
    ...
    def generate_paviment_results(results_obj, img_obj, gps_obj, min_fotogram_distance = 5, columns_to_have = ['Grieta Lineal Longitudinal','Intervalo Lineal Longitudinal','Grieta Lineal Transversal','Intervalo Lineal Transversal','Piel de Cocodrilo','Protuberancia, Bache','Otras fallas']):
        """
        Generate final table of results
        """
        ...
        height, base = img_obj.get_height_base()

        # Calculate the number of detections per frame
        data['len'] = list(map(lambda x: len(x), data.scores.values))
        
        data = data.loc[data.len>0].reset_index(drop=True)

        data_resulting = stack_columns_dataset(data, ['classes','scores','boxes'], [ 'latitude', 'longitude','distances','ind','frame','section'])
        
        # Adding new variables
        data_resulting['class_id'] = data_resulting.classes.values
        data_resulting['area'] = list(map(lambda x: area_calc(x,height, base), data_resulting.boxes.values))
        data_resulting['center'] = list(map(lambda x: box_center(x, height, base), data_resulting.boxes.values))
        data_resulting['height'] = list(map(lambda x: box_height(x,height), data_resulting.boxes.values))
        data_resulting['width'] = list(map(lambda x: box_width(x,base), data_resulting.boxes.values))
        data_resulting['total_area'] = base*height
        data_resulting['perc_area'] = data_resulting.area.values/data_resulting.total_area.values
        #adding new area calculation as well
        data_resulting['area_meters'] = list(map(lambda x: area_calc_meters(x,height, base, 3.2), data_resulting.boxes.values))
        
        # Aggregating the data
        data_resulting_g = data_resulting.groupby(['latitude','longitude','frame','distances','classes','section']).aggregate(first_aggregation_dict).reset_index().rename({'ind':'quantity'},axis=1).sort_values(['frame']).reset_index(drop=True)
        ...
        # Generating the Fail ID
        variables = list(data_resulting.classes.unique())
        data_resulting = fail_id_generator(data_resulting, min_fotogram_distance)

        # Grouping the data by sections
        data_resulting_g_sections = data_resulting.groupby(['class_id','classes','frame','latitude','longitude','fail_id_section']).aggregate(third_aggregation_dict).reset_index().sort_values(['frame'])
        data_resulting_g_sections2 = data_resulting.groupby(['class_id','classes','frame','latitude','longitude','section']).aggregate(third_aggregation_dict).reset_index().sort_values(['frame'])
        ...
        # Generating the final results datasets
    ...
    def generate_final_results_signal(results_obj, gps_obj, classes_names_yolo_signal = ['ADTCIA','CIRCROJO','CRUZAMLLA','CUADBCO','CUADCAFE','CUADVERDE',
        ...
        frame = list(range(len(BOXES_SIGNAL)))
        ...
        # quantity de frames a sacar repetidas.
        N_fotogram = 5
