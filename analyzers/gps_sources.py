...
from pavimentados2.analyzers.utils import total_distance
...
class GPS_Standard_Loader(GPS_Processer):
    ...
    def load_gps_data(self):
        ...
        for value in data:
            output = gps.get(value.sentence_type, None)
            if output == None:
        ...
        self.gps_df['timestamp'] = self.gps_df['timestamp'].astype(str).str.replace(':', '')
...
class GPS_CSV_Loader(GPS_Processer):
    ...
    def load_gps_data(self):
        ...
        if self.columns_names == 3:
            ...
            self.gps_df['timestamp'] = list(
                map(lambda x, y: dt.datetime.strptime(x + ' ' + y, '%Y-%m-%d %H:%M:%S'), self.gps_df['date'].values,
...
class GPS_Image_Route_Loader(GPS_Processer):
    ...
    def load_single_value(self, img_path):
        ...
        return {'timestamp': time, 'longitude': longitude2, 'latitude': latitude2}
...
def GPS_Data_Loader(source_type, gps_in, **kwargs):
    if source_type not in gps_source_options_dict:
        raise NameError(f'{source_type} not implemented in the method')
