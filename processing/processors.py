...
import numpy as np
import time
from pavimentados2.models.structures import Yolo_Model, Siamese_Model, State_Signal_Model
from pavimentados2.image.utils import transform_images
from pavimentados2.configs.utils import Config_Basic
...
class MultiImage_Processor(Group_Processor):
    ...
    def _process_batch(self, img_batch, video_output=None, image_folder_output=None):
        ...
        if (video_output is not None) or (image_folder_output is not None):
            for i, img in enumerate(img_batch):
                ...
                img = draw_outputs(img, ([boxes_pav[i]], [scores_pav[i]], [classes_pav[i]]))
                
                if video_output is not None:
                    video_output.write(img)
                
                if image_folder_output is not None:
                    #print(img_obj)
                    timestamp = time.strftime("%Y%m%d-%H%M%S")
                    image_filename = f"image_{i+1}_{timestamp}.jpg"  # Unique identifier with timestamp
                    image_path = os.path.join(image_folder_output, image_filename)
    ...
    def process_images_group(self, img_obj, batch_size = 8, video_output_file=None, image_folder_output = None):#image_type = 'routes', batch_size = 8):
        ...
        if video_output_file:
            ...
            print(fourcc)
            video_output = cv2.VideoWriter(video_output_file, fourcc, 3, (base, height))
            #video_output = cv2.VideoWriter(video_output_file,0, 3, (base,height))
        else:
            video_output =  None
